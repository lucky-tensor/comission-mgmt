#!/usr/bin/env bun
/**
 * provision.ts — Idempotent GCP resource provisioning for commission-mgmt.
 *
 * Creates (or reuses) VPC network + subnetwork, firewall rules, private service
 * access peering, AlloyDB cluster + primary instance (Postgres 15), and a
 * Compute Engine VM (Ubuntu 24.04, e2-standard-4, 50GB disk, SSH key injection).
 *
 * Usage:
 *   bun run provision --project <project-id> --region <region> --zone <zone> --environment <env>
 *
 ...
 *   Fallback: GCP_SERVICE_ACCOUNT_KEY_JSON, GCP_SERVICE_ACCOUNT_KEY_FILE,
...
const helpText = `
Create or reuse the Google Cloud infrastructure for a commission-mgmt deployment.
...
Common optional settings:
  --vm-name / GCP_VM_NAME                     default: commission-<env>-vm
  --network / GCP_NETWORK_NAME               default: commission-<env>
  --subnetwork / GCP_SUBNETWORK_NAME         default: commission-<env>-<region>
  --subnetwork-cidr / GCP_SUBNETWORK_CIDR    default: 10.42.0.0/24
  --alloydb-cluster / GCP_ALLOYDB_CLUSTER    default: commission-<env>-db
  --alloydb-instance / GCP_ALLOYDB_INSTANCE  default: commission-<env>-primary
...
export async function main(): Promise<void> {
  ...
  const alloyCluster = resolveOption(
    ...
    'alloydb-cluster',
    ['GCP_ALLOYDB_CLUSTER'],
    `commission-${environment}-db`,
  )!;
  const alloyInstance = resolveOption(
    args,
    'alloydb-instance',
    ['GCP_ALLOYDB_INSTANCE'],
    `commission-${environment}-primary`,
  )!;
  ...
  try {
    ...
    console.log(`Namespace:         commission-${environment}`);
  } finally {
    sshAuth.cleanup();
