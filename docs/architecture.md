...
| Web framework / UI | **React** (latest stable) + **Tailwind CSS v4** with `@tailwindcss/vite` and **Atlas design system** token bridge; React hooks + minimal context for state; native `useState` forms; thin (<50-line) typed `fetch` wrapper | Data-dense role-scoped surfaces (portal, review queue, dashboards); no CSS-in-JS, no Redux/MobX/Zustand, no form/HTTP-client libraries. Atlas (`packages/ui/design-system/atlas/`) bridges design tokens from the Figma system. | IMPL-ARCH-003/004/005, IMPL-UX-008/009/011/013 |
...
| **`@tailwindcss/vite`** | Tailwind CSS v4 bundler integration | IMPL-UX-009 | Vite plugin for Tailwind v4; ships with design tokens from the Atlas system. |
| **`@fontsource-variable/geist`** | Variable font for UI typography | IMPL-UX-008 | Atlas-coordinated typography; Geist variable font for consistent cross-role display. |
...
| **React / Tailwind CSS** | UI framework + styling | IMPL-ARCH-003/004, IMPL-UX-008/009 | Design tokens bridge from Atlas design system (`packages/ui/design-system/atlas/`) via `@tailwindcss/vite` and Geist typography; component docs are static build output (no Storybook). |
...
| `blueprints/worker.yaml` | Read-only DB, write-through-API, atomic claim, delegated single-use tokens, dual attribution, distroless, network policy; **AI-vendor rules** — HTTP path `packages/db/src/claude-api-client.ts` (`callClaudeAPI`) is a shipped-but-stubbed seam reserved for the arbitration path (#186), not yet invoked by any worker; **vendor CLI binary spawning IS in scope and shipped** — the Producer Deal Simulator engine `packages/db/src/claude-cli-engine.ts` (`runClaudeCli`) spawns the local `claude` CLI binary via `Bun.spawn` (#262, shipped in #267) and makes NO outbound vendor HTTP call; threats: `vendor-cli-data-exfiltration` (CLI path — now-realised/live for the simulation worker, mitigated by a bounded subprocess timeout, no tool/permission flags passed, and delegated single-use result tokens) and `vendor-api-key-leak` (HTTP path — present-but-latent, becomes live only when the arbitration path lands); wired in #186 (arbitration) and the Producer Deal Simulator phase (#262) | Digital-twin (P-007/D-006) |
