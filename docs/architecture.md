...
| `blueprints/worker.yaml` | Read-only DB, write-through-API, atomic claim, delegated single-use tokens, dual attribution, distroless, network policy; **AI-vendor rules** — HTTP path `packages/db/src/claude-api-client.ts` (`callClaudeAPI`); **vendor CLI binary spawning IS in scope and shipped** — the Producer Deal Simulator engine `packages/db/src/claude-cli-engine.ts` (`runClaudeCli`) spawns the local `claude` CLI binary via `Bun.spawn` (#262, shipped in #267); active, now-realised threats: `vendor-api-key-leak` (HTTP path) and `vendor-cli-data-exfiltration` (CLI path — live for the simulation worker, mitigated by a bounded subprocess timeout, no tool/permission flags passed, and delegated single-use result tokens); wired in #186 (arbitration) and the Producer Deal Simulator phase (#262) | Digital-twin (P-007/D-006) |
