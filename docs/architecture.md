...
| Web framework / UI | **React** (latest stable) + **Tailwind CSS v4** (via `@tailwindcss/vite`); React hooks + minimal context for state; native `useState` forms; thin (<50-line) typed `fetch` wrapper; **Atlas design system** (`packages/ui/design-system/atlas/`) as the token source, bridged into Tailwind v4's `@theme` layer via `apps/web/src/index.css`; Geist variable font self-hosted via `@fontsource-variable/geist` | Data-dense role-scoped surfaces (portal, review queue, dashboards); no CSS-in-JS, no Redux/MobX/Zustand, no form/HTTP-client libraries. | IMPL-ARCH-003/004/005, IMPL-UX-008/009/011/013 |
...
| **React / Tailwind CSS v4** | UI framework + styling | IMPL-ARCH-003/004, IMPL-UX-008/009 | **Atlas design system** (`packages/ui/design-system/atlas/`) is the token source; tokens are bridged into the Tailwind v4 `@theme` layer in `apps/web/src/index.css` via `@tailwindcss/vite`. Component docs are static build output (no Storybook). |
| **`@fontsource-variable/geist`** | Self-hosted variable font (Geist) | IMPL-UX-008 | **Buy** — replaces system-font stack; Geist Mono variant also included (`@fontsource-variable/geist-mono`). |
| **`@tailwindcss/vite`** | Tailwind v4 Vite plugin | IMPL-UX-008/009 | **Buy** — official Tailwind v4 first-party integration; supersedes PostCSS config approach. |
