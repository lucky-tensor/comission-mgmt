...
- **Type:** threat
- **Applicable:** yes
- **Technology implication:** An external-AI-vendor HTTP/SDK seam exists — `callClaudeAPI` (`packages/db/src/claude-api-client.ts`, #188) — reserved for the arbitration path (#186) for short structured prompts. It is a shipped-but-stubbed client (the file is an explicit dev-scout stub that does not invoke the real API), and the only worker that would call it, the arbitration worker (`apps/worker/src/agents/arbitration.ts`), is itself still a stub with the `callClaudeAPI` call commented out. The shipped simulation worker does NOT use this path. So the Anthropic-API-key HTTP surface is present-but-latent: when the arbitration path lands, the key must be injected as a K8s Secret, scoped minimally, never logged, and rotated, with egress restricted to the declared vendor host.
- **Risk:** A leaked vendor API key would let an attacker run AI requests on the firm's account (a billing/abuse vector). This becomes a live key surface only once the arbitration path actually calls `callClaudeAPI`; today no shipped worker makes an HTTP vendor call, so it is a planned surface to govern ahead of #186, not a currently-live one.
...
- **Type:** checklist
- **Applicable:** yes
- **Technology implication:** Now that the simulation worker invokes the `claude` CLI via `runClaudeCli`, each vendor CLI invocation must produce a structured audit entry — correlated by `taskId` to its `simulation_run` / `task_queue` row — into `commission_audit`, alongside the existing task-claim and result-submission entries. The same audit obligation will apply to `callClaudeAPI` once the arbitration path (#186) begins invoking the HTTP seam.
...
- **Type:** checklist
- **Applicable:** yes
- **Technology implication:** The Anthropic API key that `callClaudeAPI` (`packages/db/src/claude-api-client.ts`) will read once the arbitration path (#186) invokes it must be injected as a K8s Secret, rotatable without a code change, and never written to logs. (No shipped worker reads this key today — `callClaudeAPI` is still a stub — so this checklist gates the arbitration rollout.) Verify rotation swaps the Secret cleanly and old keys stop working.
...
- **Type:** checklist
- **Applicable:** yes
- **Technology implication:** Egress must be restricted to the API, KMS, and the declared vendor host(s), with all other egress blocked and logged via k3s NetworkPolicy. The live driver today is the CLI-spawn path (`runClaudeCli`) in the shipped simulation worker — its subprocess egress shares this boundary. The HTTP path (`callClaudeAPI`) would add an outbound Anthropic vendor-host connection once the arbitration path (#186) lands; the egress allowlist should already account for it.
- **Risk:** Unrestricted egress is an exfiltration path for the financial data the worker reads — realised today via the vendor CLI invocation (WORKER-T-010), and additionally via the HTTP vendor call once the arbitration path begins making it.
...
- **Type:** antipattern
- **Applicable:** yes
- **Technology implication:** The shipped simulation worker builds a Claude CLI prompt from financial context (and the arbitration worker will once #186 lands), so the rule is concrete: the audit log stores hashes and metadata, never the full prompt or financial content; sensitive content lives in the task record under field-level encryption and access control.
...
- Digital-twin simulation is shipped, not deferred: the Producer Deal Simulator (#262/#267) runs forecasts in an isolated digital twin (`apps/worker/src/agents/simulation.ts`) by spawning the local `claude` CLI via `runClaudeCli` (`packages/db/src/claude-cli-engine.ts`, array-form `Bun.spawn`, hard timeout, no tool/permission flags) — it makes NO outbound AI-vendor HTTP call. That makes `vendor-cli-data-exfiltration` (WORKER-T-010) a now-active threat, governed by egress restricted to the API/KMS/declared vendor hosts, hash-only audit logging of every CLI invocation, and the twin-isolation boundary — WORKER-P-007, WORKER-D-004, WORKER-D-006, WORKER-T-010, WORKER-C-007, WORKER-C-008, WORKER-C-024. The HTTP/SDK seam `callClaudeAPI` (`packages/db/src/claude-api-client.ts`, #188) is a shipped-but-stubbed client reserved for the arbitration path (#186); no shipped worker calls it yet, so the `vendor-api-key-leak` threat (WORKER-T-009) and its K8s-Secret key injection + rotation requirement (WORKER-C-017) are planned controls to land with the arbitration path, not currently-live ones.
