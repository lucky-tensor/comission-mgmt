...
describe('Atlas design-system adherence', () => {
  ...
  it('enforces Atlas radius semantics', () => {
    expect(findings(/\brounded-2xl\b/)).toEqual([]);
    expect(findings(/\brounded-lg\b/)).toEqual([]);

    const fullRadius = findings(/\brounded-full\b/);
    expect(fullRadius).toEqual([expect.stringContaining('AttributionTimeline.tsx:')]);
    expect(fullRadius[0]).toContain('w-6 h-6 rounded-full');
  });

  it('allows inline style only for data-driven chart widths', () => {
    const inlineStyles = findings(/\bstyle\s*=/);
    expect(inlineStyles).toHaveLength(2);
    expect(inlineStyles[0]).toContain('ExecTrends.tsx:');
    expect(inlineStyles[0]).toContain('style={{ width: `${pct.toFixed(1)}%` }}');
    expect(inlineStyles[1]).toContain('TierProgress.tsx:');
    expect(inlineStyles[1]).toContain('style={{ width: `${pct}%` }}');
  });
});
