...
export function cleanupStaleContainers(): void {
  ...
  console.info(`[cleanup] scanning for stale container sentinels at ${repoRoot}`);
          ...
          if (content.startsWith('{')) {
            ...
            for (const proc of sentinel.processes) {
              try {
                ...
                spawnSync('docker', ['stop', proc.containerId], { encoding: 'utf8' });
              } catch {
            ...
            spawnSync('docker', ['stop', content], { encoding: 'utf8' });
          }
  ...
  for (const process of sentinel.processes) {
    try {
      ...
      spawnSync('docker', ['stop', process.containerId], { encoding: 'utf8' });
    } catch {
      // Container may already be gone
    }
  }
