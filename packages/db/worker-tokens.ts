 ...
 *
 * Scoped single-use worker token store for commission management tasks.
 *
 * Each token is minted for a specific pod + commission task scope combination.
 ...
 * Security model
 * ---------------
 * - A token is bound to one pod_id, one task_id, and one pod_id scope.
 ...
 *
 * Commission-specific scope fields
 * ---------------------------------
...
export interface WorkerTokenRow {
  ...
  pod_id: string;
  agent_type: string;
  /** Commission task scope: task_id from task_queue bound to this token. */
  task_id: string;
  jti: string;
  expires_at: Date;
...
export interface MintWorkerTokenOptions {
  podId: string;
  agentType: string;
  /** Commission task ID this token is scoped to (from task_queue.id). */
  taskId: string;
...
export async function persistWorkerToken(options: MintWorkerTokenOptions): Promise<WorkerTokenRow> {
  ...
  const rows = await db<WorkerTokenRow[]>`
    INSERT INTO worker_tokens (pod_id, agent_type, task_id, jti, expires_at)
    VALUES (${options.podId}, ${options.agentType}, ${options.taskId}, ${options.jti}, ${expiresAt})
    RETURNING *
  `;
  return rows[0];
}
